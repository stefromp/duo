...
# Try to import FlashAttention, but make it optional for Kaggle compatibility
try:
    import flash_attn
    ...
    print("Falling back to PyTorch native attention (SDPA)")
...
def apply_rotary_emb_torch_fallback(x, cos, sin):
  ...
  cos = cos.unsqueeze(0).unsqueeze(2)  # [seq_len, head_dim//2] -> [1, seq_len, 1, head_dim//2]
  ...
  cos_full = torch.cat([cos, cos], dim=-1)  # [1, seq_len, 1, head_dim]
...
def split_and_apply_rotary_pos_emb(qkv, rotary_cos_sin):
  with torch.cuda.amp.autocast(enabled=False):
    ...
    if FLASH_ATTN_AVAILABLE:
      ...
      q = apply_rotary_emb_torch_fallback(
        q.squeeze(dim=2), cos, sin)
      k = apply_rotary_emb_torch_fallback(
        k.squeeze(dim=2), cos, sin)
...
def apply_rotary_pos_emb(qkv, cos, sin):
  ...
  if FLASH_ATTN_AVAILABLE:
    ...
    # Fallback: apply rotary embedding to each component
    # qkv shape: [batch, seq_len, 3, num_heads, head_dim]
    q, k, v = qkv[:, :, 0], qkv[:, :, 1], qkv[:, :, 2]
    
    # Reshape for rotary application: [batch, seq_len, num_heads, head_dim] -> [batch*num_heads, seq_len, head_dim]
    batch, seq_len, num_heads, head_dim = q.shape
    q_flat = q.reshape(-1, seq_len, head_dim)
    k_flat = k.reshape(-1, seq_len, head_dim)
    
    # Apply rotary embeddings
    q_rot = apply_rotary_emb_torch_fallback(q_flat, cos, sin)
    k_rot = apply_rotary_emb_torch_fallback(k_flat, cos, sin)
    
    # Reshape back
    q_rot = q_rot.reshape(batch, seq_len, num_heads, head_dim)
    k_rot = k_rot.reshape(batch, seq_len, num_heads, head_dim)
    
    # Stack back into qkv format
    qkv[:, :, 0] = q_rot
    qkv[:, :, 1] = k_rot
...
class DDiTBlockCausal(nn.Module):
  ...
  def forward(self, x, rotary_cos_sin, **kwargs):
    ...
    if FLASH_ATTN_AVAILABLE:
      # Use FlashAttention for efficient attention computation
      x = flash_attn.flash_attn_interface.flash_attn_varlen_qkvpacked_func(
        qkv, cu_seqlens, seq_len, 0.0, causal=True)
    else:
      # Fallback to PyTorch native attention (SDPA)
      # Reshape qkv: [(b*s), 3, h, d] -> separate q, k, v
      q, k, v = qkv[:, 0], qkv[:, 1], qkv[:, 2]
      # q, k, v shape: [(b*s), h, d]
      ...
      # x shape: [b, h, s, d] -> [b, s, h, d] -> [(b*s), h, d]
      x = x.transpose(1, 2).reshape(batch_size * seq_len, self.n_heads, -1)
